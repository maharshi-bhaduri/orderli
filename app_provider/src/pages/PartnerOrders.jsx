...
export default function PartnerOrders() {
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .on(
        ...
        (payload) => {
          const localCache =
            JSON.parse(localStorage.getItem("ordersToUpdate")) || {};
          ...
          setOrders((prev) => {
            ...
            if (payload.eventType === "INSERT") {
              ...
              if (updatedOrders[key]?.items) {
                updatedOrders[key].items = updatedOrders[key].items.map(
                  (
                    order // error at this line
                  ) =>
              ...
              if (updatedOrders[key]) {
                ...
                if (updatedOrders[key].items.length === 0)
  ...
  const toggleItemStatus = (orderKey, orderItemId, currentStatus) => {
    ...
    setOrders((prev) => {
      const updatedOrders = { ...prev };
      updatedOrders[orderKey].items = updatedOrders[orderKey].items.map(
        (item) =>
      ...
      onSuccess: (_, variables) => {
        ...
        setOrders((prevOrders) => {
          ...
          for (const key in updatedOrders) {
            ...
            if (updatedOrders[key].items.length === 0)
              delete updatedOrders[key]; // Remove empty orders
          }
          return updatedOrders;
        });
      },
  ...
  const markOrderComplete = async (orderKey) => {
    ...
    const { error } = await supabase
      ...
      .in(
        "orderItemId",
        orderItems.map((item) => item.orderItemId)
      );
      ...
      {Object.keys(orders).length > 0 ? (
        <div className="flex gap-4 flex-wrap p-2">
          {Object.entries(orders).map(([orderKey, order]) => {
            const allChecked = order.items.every(
              (item) => item.itemStatus === 2
            ); // ✅ Check if all items have itemStatus === 2
              ...
              >
                ...
                <div className="flex justify-between items-center mb-2">
                  <div className="flex items-center">
                    ...
                    <span
                      className={`text-sm mx-2 px-2 py-1 bg-gray-200 text-gray-700 rounded-full border
                                         ${
                                           isUrgent
                                             ? "bg-red-400 text-white border-red-500 border-2"
                      ...
                      >
                        {item.itemStatus === 2 && (
                          ...
                          >
                            <path
                              fillRule="evenodd"
                              d="M20.285 6.707a1 1 0 0 0-1.414-1.414L9 15.164l-4.293-4.293a1 1 0 1 0-1.414 1.414l5 5a1 1 0 0 0 1.414 0l10-10z"
                              clipRule="evenodd"
                            />
                          </svg>
                        )}
